...
class LocalsControllerTest < ActionController::TestCase
  ...
  test "should create local" do
    assert_difference('Local.count') do
      post :create, local: { description: @local.description, name: @local.name, owner_id: @local.owner_id }
    end
  ...
  test "should update local" do
    patch :update, id: @local, local: { description: @local.description, name: @local.name, owner_id: @local.owner_id }
    assert_redirected_to local_path(assigns(:local))
  end
